'use strict';
/* Golf Handicap — the offline service worker.
   ==========================================================================
   Ported from the financial-plan repo, by way of Sprint Velocity and Flow
   Metrics, where the design and its objections are recorded at length. The two
   facts that make this the most security-sensitive file here are worth
   repeating rather than linking to:
   ...
   2. CACHE STORAGE IS ORIGIN-WIDE, NOT PER APP. Every app in the family shares
      ...
      (`fin-shell-`, `sv-shell-`, `td-shell-`) sit in the same store. The answer
      is the rule below: only files that are ALREADY PUBLIC in this repo are
      ever cached. Nothing an attacker could read from the cache
      is anything they couldn't read from GitHub. The rounds stay in
   ...
   The scope is this file's own directory. Widening it past `/golf-handicap/`
   ...
   running here, the app refuses to open it rather than letting normalizeState()
   rebuild it without the fields that build added — and push the loss back to
   the cloud on the next save.
...
/* THE ALLOWLIST, and the security boundary of this file. Every entry is a file
   ...
   the rounds, not a Firestore reply, not tests.html, not example-league.json,
   not the repo's own notes. A request that is not on this list is not
...
/* Resolved against this file's own URL, so the same list works unchanged on
   ...
   /golf-handicap/). Don't hard-code the directory. */
