...
- **The header and the page width are Team Dashboard's and Sprint Velocity's.** A sticky bar
  ...
  top) is inline text inside the heading, not that element back. Below 560px the bar is deliberately **not sticky** — seven
  controls wrapped to four lines on a phone, and a sticky bar that deep eats half the screen on
  the one view you scroll most. Since 2026-09-14 they are one scrolling line under the name
  (see the one-scrolling-line note at the top) and the bar is two lines deep there, not five;
  it was left un-sticky all the same, since nobody asked for that to change.
