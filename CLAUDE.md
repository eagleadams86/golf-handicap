...
- **⌕ Find / ⌘K is Money Map's window, ported (2026-08-23)** — `searchApp(st, query)` is PURE over the state it is handed and does NOT use `courseById`, which is bound to the live `state`; a search over a test fixture has to be a search over that fixture. Two-character minimum, `SEARCH_CAP` of 80 with the overflow COUNTED so the cap is never silent, and the count span (`role="status"`) is the live region rather than the results list. A round matches on its note, its course, its tees, its golfer and its date **as stored** — which is why `2026-08` is a month and `2026` is a season, and why the date must stay ISO in the hit's `where`. `renderSearchResults` fences a shared view down to golfers and rounds (no Golfers & Courses window for a visitor) and `goToSearchHit` navigates without opening the round card there, since nothing would save. Mirrored into Sprint Predictability and Flow Metrics in the same pass — a change to one belongs in all three — and the Task Dashboard has one too, which makes six windows in five apps plus the starter.
...
- **The Find block sits OUTSIDE `@media (min-width: 760px)`, and that is not tidiness.** It was ported into that block on 2026-08-23, dropped between two window WIDTHS where it looked at home — and a width is the only thing in there that can be conditional. Everything else the window says about itself (its padding, the box, the hit and its three lines) simply stopped applying below 760px, which is a phone: the results list fell back to this app's default buttons, at control height, in 16px. Found 2026-08-23 while making the six windows one. Nothing about the Find window belongs in a width query.
...
- **`#syncBtn`'s width floor is in em, and that is not a style choice (2026-09-12).** The sync button is the one control in the header whose LABEL changes — "☁️ Sign In to Sync" becomes "☁️ Charlie" the moment Firebase answers — so without a floor under its widest state every control to its right slides sideways on every refresh. The floor has now gone stale TWICE as a pixel number, and for two different reasons. 125px was measured at an older type size and stopped covering the label when the pack moved to a 16px rem ramp on 2026-08-22 (raised to 134 on 2026-08-26). Then 134px turned out to cover only the MOUSE size: `.headbar button` takes `--fs-base` on a coarse pointer, so the same label is 126.98px with a mouse and 148.17px on a phone, and the header still slid ~14px on every phone refresh — under a floor written to stop exactly that. **No single pixel number can cover two type sizes.** `min-width: 10.25em` is measured in the button's own type, so it follows the coarse step and any future ramp without being touched. The figure is this app's own: the widest label needs 9.77em at the mouse size, which is the BINDING case because the 9px of padding is a bigger share of a 13px em than of a 16px one, and 9.26em on touch. League Night answered this first (`c3a8376`); Money Map carries the same block, and PAPTrack still carries a pixel pin. **tests.html measures both type sizes against the floor and pins the unit** — the coarse size is reached by putting `--fs-base` on the button, because pointer media cannot be emulated from script and the old test was green in a desktop-width frame for the whole life of the bug.
- **The header's controls and the tab bar are ONE SCROLLING LINE AT EVERY WIDTH (2026-09-14, family-wide).** Charles: *"should we just make them both always single line side scrollers?"* → *"go ahead, do the whole family"*. Money Map went first (`b14f4ae` in claude-financial-plan) and this is its shape, to its numbers, in this app's class names and tokens. **The header:** `.headrow` holds `.headctl`, the scroller (`flex-wrap: nowrap; overflow-x: auto`, 4px padding and a `-4px` margin for the focus ring, no scrollbar), and a `.rownav` of two arrows. The row takes what the name leaves and scrolls THERE (see the next bullet); in this first cut it was content-sized (`flex: 0 1 auto`), which took the whole row onto a line of its own under the name the moment it did not fit. The family's header ORDER is unchanged and still pinned by the Find test, whose `parentElement` is now `.headctl`. **The tabs:** `.tabrow` (new, takes the bar's 20px below) holds the tablist — the scroller now — and a `.rownav no-print`; the arrows cannot sit inside a tablist, where a screen reader would count them as tabs. Two tabs fit almost anywhere, so the tab arrows essentially never show today; the bar is the family's shape kept whole for the day a third tab arrives. **`wireScrollRow(row, nav)`** is Money Map's verbatim: arrows shown only while the row overflows (ResizeObserver + MutationObserver + scroll + resize; nothing calls it after boot), each disabled at its own end, a press steps 80% of the row, instantly. **The arrows are for a mouse or trackpad only** (`@media (hover: none), (pointer: coarse) { .rownav { display: none !important } }` — on touch they grew Money Map's header), `tabindex="-1"` + `aria-hidden`. Their style is `.rownav button, .headbar .rownav button` — the two-class half out-ranks `.headbar button`, which reaches the header's pair and would otherwise make them bordered 30px chrome buttons. Anything that hides the tab bar hides `.tabrow` now (the `data-welcome` gate, `renderShareError`), so the arrows never stand alone. Print wraps `.headctl` instead of scrolling it. **Measured against `501d62f`:** 1600px pixel-identical (fresh and with the sample league); every other window had WRAPPED and is now one line — 1100px header 93px → 88.5, 705px 135 → 88.5 (arrows shown), a 390px phone 256 → 118, a sideways 844×390 phone 101 → 92.5; the tabs were already one row everywhere. **Not identical at 1100px in that first cut:** that window used to fit five controls beside the name and three below; the whole row went below the name, which moved the name up 2.3px — superseded the same evening by the next bullet. tests.html measures a 705px and a 1600px frame in `onAppLoad` (`ROWS`), narrows the tab row to make the tab arrows appear, and was confirmed red on the old page.
- **Beside the name, whole on focus, and pinned on a phone (2026-09-14, the same evening).** Charles: *"keep buttons beside the name"*, *"make golf sticky again"*, and *"fix those four issues"* (the first: a control part-way past a row's edge stayed clipped when reached from the keyboard). Money Map's `2c19e01`, followed here. **(1)** `.headrow { …; justify-content: flex-end; flex: 1 1 0%; min-width: min(15rem, 100%) }` — the row takes what the name leaves, controls packed right, and wraps under the name only when that would leave it under 15rem. With this app's long name that is an upright phone and nothing else: the name takes two lines there and the row one line below. The row itself never overflows (the scroller inside shrinks), so `flex-end` cannot strand a control. **(2)** `wireScrollRow` gains Money Map's `focusin` listener, verbatim — the helper is still byte-identical across the family: a `:focus-visible` focus on a control part-way past either PADDING edge scrolls it wholly in, because the browser only scrolls a WHOLLY hidden focus target. **(3)** The `@media (max-width: 560px)` block no longer sets `header { position: static }`; that exception was written for four wrapped lines of controls, which are gone. **(4) Pinning the phone bar exposed one more thing, fixed with it:** `html { scroll-padding-top: calc(var(--head-h, 0px) + 8px) }`, where `--head-h` is the header's height from a ResizeObserver at the foot of the page. Without it, Shift+Tab back up a 390px phone landed 27 of 91 stops wholly behind the 118px bar — and a sideways 844×390 phone, sticky all along, 35 of 87. Both 0 now (the skip link is the only thing over the bar, as on a desktop). `--maxi-top` is separate and unchanged: it is measured when the chart goes full screen. **Measured against `33f14d0`** (Playwright, sample league loaded): 1600px header 51 → 51, identical; 1100px 88.5 → 51 and 705px 88.5 → 51, controls beside the name and scrolling; 844×390 phone 92.5 → 55, beside the name; 390×844 phone 118 → 118, row under the two-line name, and after `scrollTo(0, 800)` the header's top is 0 (was −800, `static`). Real Tab / Shift+Tab walks: before, League Rules 7px and the golfer picker 90px clipped at 705px, Share 48px and Find 21px at 844×390, and the Leaderboard tab 68px along a 260px tab row by arrow key; after, nothing. **Tests** (`ROWS`): beside the name at 705px (plus a source pin of the rule); a keyboard reveal on the header AND the tab bar — a control's middle scrolled onto the edge, then focused; the tab row is set to 260px with the Leaderboard tab shown, because 60px is narrower than one tab and proves nothing; a 390px frame with a 3000px spacer whose header must stay `sticky` at top 0; and a button 2000px down scrolled to the start, which must land below the header. All red on `33f14d0` (run with soft asserts, so no check hid behind an earlier one); the reveal checks red again with only the listener disabled, and the scroll-padding check red with only that rule disabled. `EXPECTED` 359 → 363.
- **`color-scheme` is the theme pack's, not this app's, since 2026-09-12 (pack rule 18).** `theme.css` emits it on every palette block — `:root`, each `[data-theme]`, BOTH halves of `auto`, and the print block — derived from each theme's `isLight` in `tokens.json`, so the pairing follows the PALETTE rather than the theme's name and sepia, a warm *light* theme, reads `light`. It is how a page tells the browser which way round it is, so browser-drawn UI follows: without it the calendar button inside `<input type="date">` is a near-black glyph on a near-black field on the dark themes, and that glyph is not restylable from CSS (number spinners, checkboxes and scrollbars have the same problem more quietly). This app declared it locally, alongside the chart colours, until the pack took it. It was hand-copied into every app until then, and by that date it had been **missed on ten pages** and had drifted into two spellings of the Auto half. **Don't re-add a local block** — it is drift now, and `check_consumers.py` flags it.
...
- **The rolling figure is called the LEAGUE handicap on screen**, and its settings dialog is
  reached from the header button **League Rules**. It used to be "My method" everywhere; the
  ...
  naming it after the owner was wrong. The rule's *own* name (`method.name`, derived by
  `autoName` — "Rolling last 5", "Best 3 of last 5" — never typed, per the derived-name
  rule above) is a different string and still appears: as the
...
- **The header and the page width are Team Dashboard's and Sprint Velocity's.** A sticky bar
  ...
  as in both siblings; don't put visible captions back. The old standalone strapline
  ELEMENT went (2026-08-11): with the courses and league-rule buttons moved up here the row
  had no room for it, and the media query that used to drop it on a narrow screen went with
  it. The fixed strapline the brand `<h1>` carries today (see the brand-line rule near the
  top) is inline text inside the heading, not that element back. **The bar is sticky at every
  width, phones included, since 2026-09-14** (Charles: *"make golf sticky again"*). From
  2026-08-11 it was `position: static` below 560px — seven controls wrapped to four lines on a
  phone, and a sticky bar that deep ate half the screen on the one view you scroll most — and
  that reason went when the controls became one scrolling line. A pinned bar needs
  `html { scroll-padding-top }` from its measured height or a focus scrolled to the top lands
  behind it; see the beside-the-name bullet at the top.
